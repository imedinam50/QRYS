...
--                        OR (wait_type = 130) -- LOGMGR_QUEUE waits 
                        OR (wait_type > 177 AND wait_type < 180) -- FCB_REPLICA_ waits 
                        OR (wait_type = 181) -- WRITELOG waits 
--                        OR (wait_type = 188) -- LOGMGR waits 
                        OR (wait_type = 189) -- CMEMTHREAD waits 
                        OR (wait_type = 191) -- CXPACKET waits 
                        OR (wait_type = 214) -- MSQL_XP waits 
                        OR (wait_type = 215) -- MSQL_DQ waits 
                        OR (wait_type = 216) -- LOGBUFFER waits 
                        OR (wait_type = 221) -- TRACEWRITE waits 
                        OR (wait_type > 426 AND wait_type < 615) -- PREEMPTIVE_ waits 
                        OR (wait_type > 931 AND wait_type < 973) -- LCK_*_LOW_PRIORITY & LCK_*_ABORT_BLOCKERS waits 
                        OR (wait_type = 284) -- RESOURCE_SEMAPHORE_MUTEX waits 
                        OR (wait_type = 298) -- RESOURCE_SEMAPHORE_QUERY_COMPILE waits 
                ) 
        ) 
), 
...
--, 
--ADD EVENT  sqlserver.query_memory_grant_blocking (--        Occurs when a query is blocking other queries while waiting for memory grant
--        ACTION ( 
--                sqlserver.database_id,--        Collect database ID 
--                sqlserver.transaction_id,--        Collect transaction ID 
--                sqlserver.session_id,--        Collect session ID 
...
--        ) 
--), 
--ADD EVENT  sqlserver.query_memory_grant_usage (--        Occurs at the end of query processing for queries with memory grant over 5MB to let users know about memory grant inaccuracies
--        ACTION ( 
--                sqlserver.database_id,--        Collect database ID 
--                sqlserver.transaction_id,--        Collect transaction ID 
--                sqlserver.session_id,--        Collect session ID 
...
--				sqlserver.session_resource_group_id --        Collect current session resource group ID
--        ) 
--        WHERE ( granted_memory_kb >= 16384 )-- 16 MB
--)--, 
...
--        ) 
--) 
ADD TARGET package0.event_file(SET filename=N'c:\xevents\long_running_events.xel',max_file_size=(1024),max_rollover_files=(4)) 
